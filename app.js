...
app.get("/api", (req, res) => {
  ...
  // Format UTC time in "2023-09-07T13:40:15Z" format
  const formattedUtcTime = currentDate.toISOString().slice(0, -1) + "Z";

  // Calculation of a random offset within +/-2 minutes for UTC time
  const offsetMinutes = Math.floor(Math.random() * 5) - 2;
  currentDate.setMinutes(currentDate.getMinutes() + offsetMinutes);
  const currentUtcTimeWithOffset = currentDate.toISOString().slice(0, -1) + "Z";
